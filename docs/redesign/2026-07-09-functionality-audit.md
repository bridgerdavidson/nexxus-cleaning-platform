...
| # | Gap | Status | Notes |
|---|---|---|---|
| R1 | **Calendar / scheduling cockpit** (month/week/day/agenda, per-cleaner dispatch lanes, drag-to-reschedule w/ conflict detection, slot-click-to-create) | ✅ done (first cut) | R1 build (2026-07-10): own model-aware "Calendar" nav tab (`/app/admin-dashboard/calendar`, gated `can_view_bookings`). Four views (`src/components/redesign/calendar/*`) built from the design system on the existing tested `src/lib/calendar/*` logic + `useAdminAppointments`. Drag opens the shipped `RescheduleDialog` (conflict detection + outcome preview reused); click opens the `?booking=` sheet; empty-slot opens the `?newbooking=` sheet seeded. Brand-blue now-line; Overdue badge reuses R10's `isResponseOverdue`. Per-cleaner dispatch lanes + availability shading are dormant seams keyed on `default_payout_model` for the future hourly model (not built this cut). Keyboard reschedule is via the event's detail-sheet Reschedule dialog (event drag is pointer-only). |
...
| R10 | **Action Center completeness**: SLA-overdue bucket + per-attempt routing/decline history | partial | Overdue bucket ✅ shipped (2026-07-10): `deriveOverview` buckets pending+awaiting bookings whose `response_deadline` passed into a "Response overdue" queue group (subtitle names the silent cleaner; Review opens the sheet in place). Remaining: per-attempt routing/decline history (a booking-sheet section; pairs with the R6/R9 sheet work). |
