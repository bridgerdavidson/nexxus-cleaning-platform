...
- **Integration-test the destructive delete executor.** `archiveOrDeleteProperty`/`countPropertyAppointments` (in `useAdminData.ts`) have no automated coverage; the branching decision (`planPropertyDeletion`) is fully unit-tested, but the multi-table executor (cancel live appts + deactivate recurring + archive/hard-delete) is verified only by review trace. No test precedent exists for the client-side anon `supabase` singleton and the sibling `deleteProperty` is also untested, so proper coverage needs a client-injection refactor. Add a local-Supabase integration test asserting the core invariant: history rows (`completed`/`cancelled`) are never mutated or deleted. (Surfaced by the Task 4 review, 2026-07-12.)
