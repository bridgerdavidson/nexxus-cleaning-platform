...
### 4.4 Active-job "Message office" (final design — Bridger-directed)
The "Message office" button in `CleanerActiveJobView` opens an **office picker bottom sheet ON the active-job page** (the reused `CleanerOfficePicker`, rendered inside `CleanerActiveJob` alongside the Directions/Skip drawers). Rationale (decided with Bridger, superseding the earlier auto-target draft): the cleaner usually wants to message a **specific** person about the job, an on-page sheet lets them dismiss and stay on the job, and it mirrors the Directions sheet. Behavior:
- Tapping it opens the sheet, listing the office (admins/managers via `useOrganizationMembers` + `filterOfficeContacts`; a spinner shows while contacts load). Dismissing keeps the cleaner on the active job (local state, **no navigation**).
- Picking a person calls `openThreadFromJob(userId, jobId)` → `/app/cleaner-dashboard/messages?to=<userId>&appointment=<jobId>&from=<jobId>`: the thread host opens the takeover, stages the job as a **"Re: <job>"** chip (reuse `useSendMessage`'s `appointmentId`, no schema change), and because `?from` is set the thread's back button reads **"Back to job"** and `router.replace`s back to `/app/cleaner-dashboard?job=<jobId>` (replace, not push, so the dismissed thread is not stranded on the back-stack where a gesture-back would re-open it).
- The button does **not** auto-target a primary contact (Bridger chose explicit recipient choice). `resolvePrimaryOfficeContact` remains in `office-contacts.ts` for potential future reuse but is not used by this button. The truly-no-office-contact org (no admin/manager) is a near-impossible state; the sheet shows a clear "No office contacts yet".
