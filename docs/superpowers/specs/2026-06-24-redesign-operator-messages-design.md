...
- **`OperatorMessages.tsx`** (outer permission gate, default export). Reads only `useAuth()` + `useManagerPermissions()`. Gates on **`can_view_messages`** (CORRECTION: messaging DOES have a `can_*` flag, and the legacy manager dashboard denies the Messages tab when it is false). `canView = privileged (owner|admin) || permissions.can_view_messages`; holds a spinner while a manager's permissions resolve (no denied flash); renders an access-denied `EmptyState` (`ShieldAlert`) when not permitted; otherwise mounts `OperatorMessagesData` (the hooks/handlers/VM container). This mirrors the Customers/Cleaners "gate before fetch" pattern so a restricted manager never mounts the messaging hooks. `OperatorMessagesData` also computes `canViewPayments = privileged || can_view_payments` to gate the lifetime-$ stat.
...
- **Permission gate: `can_view_messages`.** Owners/admins always pass; a manager needs `can_view_messages` (the legacy manager dashboard enforces the same flag). The gate mounts no data hooks when denied. `rolesUserCanMessage` is a separate concern: it only constrains *which roles* appear in New message / who can be messaged.
