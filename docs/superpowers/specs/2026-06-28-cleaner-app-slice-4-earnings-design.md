...
- `src/hooks/useCleanerConnect.ts` — add an optional `appearance` param (mirror `useTenantConnect(appearance)`); thread it into `loadConnectAndInitialize` instead of the hardcoded yellow.
...
- **The embed lives in ONE stable slot; reveal-on-click for setup (decided).** Resolved approach: render the embed via a single `<CleanerStripeConnect appearance={...} />` (after adding the appearance prop) in one fixed position in the tree. When `cleanerStatusKind === 'inactive'` (or `'pending'`) **and the embed has not been revealed yet**, show our "Get paid for your work" / "Finish payout setup" framing card with the single button instead; the button flips a local `revealed` flag that mounts the embed into that same stable slot. When `cleanerStatusKind === 'active'`, auto-reveal (mount the embed immediately, skip the button) so returning cleaners land straight on the payouts table. The only transition is **card -> embed (mount)**, never embed -> card (unmount), so the iframe is never torn down after first paint. `CleanerStripeConnect` then internally swaps `ConnectAccountOnboarding -> ConnectPayouts` when onboarding completes (the `ConnectComponentsProvider` stays mounted), so we never double-initialize and the popup `window.opener` invariant holds. Do NOT branch such that the embed renders under different parent nodes per state.
