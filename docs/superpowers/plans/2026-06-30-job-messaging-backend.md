...
## Post-review hardening (applied after a high-effort diff review)

Three backend findings were fixed in the route/helper before push (tests added; all green):
- **Kill-switch fail-open -> fail-closed.** The org-flag read now returns 503 on a read error or missing org instead of defaulting to "enabled", so a transient error can't deliver a message for an org that opted out.
- **Idempotent send.** The route accepts an optional `clientMessageId` (validated UUID) used as the message id, and inserts via `upsert(..., { onConflict: 'id', ignoreDuplicates: true })`; the `job_message` notification dedupes on the same id. A retried send (lost 201) no longer duplicates the message or notification. The PR2/PR3 client wrappers must pass a stable `clientMessageId`.
- **Send window requires cleaner acceptance.** `isJobMessagingWindowOpen` now also takes `cleaner_confirmation_status`: `confirmed` opens the window only when the cleaner is `approved` (set by accept / accept-counter / force-assign); `in_progress` and the post-completion grace remain open. This blocks messaging a cleaner who is office-confirmed but has not accepted (or rejected).

Deferred (correctly out of PR1 scope): the **office-inbox segregation** (job threads must not leak into the existing office inbox/unread badge; replies must route through this route) - this is a consumer concern that lands with PR2/PR3, see the roadmap below. Accepted (minor): the kill-switch read is a separate round trip on the human-paced send path.
...
- **PR2 - Homeowner Slice 3 (Messages tab).** Replace the stub `src/app/(redesign)/app/homeowner-dashboard/messages/page.tsx` with a sectioned inbox (Office pinned + active "Your cleanings" job threads + Past), built on a shared sectioned-inbox derive that replaces `deriveOfficeInbox`'s 1-vs-many heuristic. Reuse `MessageThreadPanel`/`MessageComposer`/`MobileTakeover`; read-only composer when the window is closed (reuse `isJobMessagingWindowOpen`). Wire "Message about this cleaning" (job send via the PR1 route) + "Message office" (existing client path) into `HomeownerCleaningDetail.tsx` (the seam after the fields card) + optionally the Home hero.
  - **REQUIRED first (office-inbox segregation - the dominant integration risk, from the PR1 code review):** the moment job messages exist they live in the shared `conversations`/`messages` tables, so the existing office consumers will leak them. Before/with shipping any send entry point, update `useConversations`, `useUnreadMessageCount`, the legacy homeowner `MessagesPage`, and the **already-shipped** redesign `CleanerMessages` to filter the Office inbox + its unread badge to `appointment_id IS NULL`; surface `appointment_id NOT NULL` (job) conversations only in the new job-thread section with their own unread count. Job-thread sends/replies **must** go through the PR1 route - the client `useSendMessage` INSERT is RLS-blocked for the homeowner<->cleaner pair (`can_message_role`), so a reply via the office path silently fails.
  - Add a `useSendJobMessage` client wrapper that POSTs the route with a client-generated `clientMessageId` (the route is idempotent on it - safe to retry) + optimistic append (mirror `useSendMessage`'s temp-id reconcile). Extend `useConversations` to surface `appointment_id` (additive). Refine the homeowner `job_message` deep-link to open the thread. Tests: inbox derivation (Office + N job threads, active/archived), read-only archived rendering. Playwright on the Vercel preview (homeowner role + redesign flag); user is on mobile, so drive the preview and send screenshots.
- **PR3 - Cleaner companion.** Add an "active job thread (to homeowner)" entry point from `CleanerActiveJob` (reuse the `useOpenOfficeThread` `?to=&appointment=&from=` plumbing, but send via the PR1 route with a `clientMessageId`) + a job-threads section in the cleaner inbox (adopt the shared sectioned derive). **Apply the same office-inbox segregation** to the cleaner side (job threads out of the Office inbox/badge). Refine the cleaner `job_message` deep-link.
