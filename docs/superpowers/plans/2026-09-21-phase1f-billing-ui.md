...
| R16 | Resolve the seat cap **inline in the invite dialog**, owner only; non-owners get "ask your account owner" with no pricing. **Exception, added 2026-09-24: the TRIAL case (Case 0) navigates to Settings > Billing instead.** It must NOT call `openPaywall()` | Jobber (our direct competitor) and Calendly both resolve inline rather than losing the invite. But Case 0 is reachable ONLY while trialing, and `paywallGate` returns hidden unless `access.frozen` (`paywallModel.ts:100`), so `openPaywall()` there is dead 100% of the time. Relaxing that gate was rejected: the paywall's headline is "Your trial has ended", which is false for a trialing org, and the frozen invariant is what the whole interstitial rests on. Inline exists so a simple seat add does not lose an invite; buying a first plan genuinely needs the larger surface |
...
| R24 | **Two Stripe portal configurations.** Owners get the full portal (tag `nexxus_portal='default'`, cancel enabled). Admins get a remediation-only one (tag `nexxus_portal='remediation'`: `payment_method_update` and `invoice_history` on, `subscription_cancel` and `subscription_update` OFF). `getOrgPortalLink` selects by the caller's role | R15 v4 lets an admin open the portal to fix a card, but our single config has `subscription_cancel: { enabled: true }` (`stripe-billing-setup.ts:226`), so an admin sent there for remediation could END the agreement. That makes the owner-only rule decorative. Turning cancel off entirely was rejected because cancellation must be no harder than signup (ROSCA, California ARL) and we have no other cancel flow |
