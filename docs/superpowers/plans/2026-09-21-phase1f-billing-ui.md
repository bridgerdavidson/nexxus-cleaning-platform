...
| R15 | **Clarified 2026-09-22 (v3), after Task 8 flagged the wording as ambiguous.** Three distinct audiences, do not collapse them: (a) the trial **PILL** renders for owner AND admin, because it is information; (b) every **ACTION** (Choose a plan, Extend, Update payment method) is **owner only**, so an admin sees a banner with an empty actions array; (c) the frozen bar's **EXPLANATION** additionally reaches MANAGERS with no actions at all (that is R2). Cleaners are on a different shell and see none of it | v2 read "the pill and every pay CTA are owner + admin", which implies an admin gets clickable buttons. That contradicts Bridger's explicit call that only the owner sees CTAs. GitLab shipped a trial banner to all users and regretted it; but a manager who cannot create a booking must still learn why, or we rebuild the dead-control failure this design exists to prevent |
