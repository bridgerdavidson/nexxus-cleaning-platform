...
|---|---|---|---|
| H1 | `clawbackCleanerPayout` has no `bank_paid` guard (reversing after funds reached the cleaner's bank drives the connected balance negative) and reverses the full `payout.amount` without reading `transfer.amount_reversed` (over-ask after a partial-refund reversal = permanent `cleaner_clawback_failed` retry loop). | `clawback.ts:98-116` | **FIXED-B** (`bank_paid` surfaces `clawback_blocked_bank_paid` + deduped `clawback_blocked` notification, never auto-reverses; reversal capped at `transfer.amount - amount_reversed` via `retrievePlatformTransfer`, fully-reversed transfers mirror `reversed`) |
| H2 | `charge.refunded` processed before `payment_intent.succeeded`: reversal no-ops (no transfers yet), settlement then pays cleaner/tenant, never clawed back. Plus the succeeded handler clobbers a `refunded` row back to `paid`. | `dispatchStripeEvent.ts:136-144,614` | **FIXED-B** (succeeded handler leaves a `refunded` row alone and skips settlement; both settle paths read `charge.amount_refunded` via `refundGuards` — fully refunded skips + retires retryable payout rows, partial refunds shrink the split base) |
| H3 | `refund.updated` -> `failed`/`canceled` updates the `refunds` row only; `payments.status` stays `refunded` forever (money went back to the merchant, UI says refunded). | `dispatchStripeEvent.ts:416-455` | **FIXED-B** (recomputes coverage from pending+succeeded refunds, reverts the payment to `paid`, deduped `refund_failed` admin notification) |
| H4 | `retryFailedPayouts` re-settles with key `cleaner-payout-{id}`; a legacy-path payout used `payout-{id}`, so a retry creates a second transfer (double-pay). | `reconcile.ts:246-272` | **FIXED-B** (settle repairs — never re-transfers — a retryable row already carrying a `stripe_transfer_id`; the sweep filters `stripe_transfer_id IS NULL` + `payout_percent_snapshot IS NOT NULL`; terminal payout states short-circuit the cleaner leg) |
...
- **M4** webhook-driven notifications duplicate on event reprocess. Column + unique index shipped in 088 and ALL new Phase A events use it. **FIXED-D** (pre-existing call sites wired: `cleaner_paid` keyed on payout id, `dispute_opened` on dispute id, both `authorization_failed` sites on appointment + PaymentIntent id).
...
- **M7** disputes UI absent: `disputes` is written + bell-notified but nothing reads it; a disputed charge still renders "Paid" (v2's G10). **FIXED-D** (`useDisputes` org-scoped hook; Disputes card in the Finance page with status/reason/respond-by; an OPEN dispute overrides the payment row's chip to "Disputed" and suppresses the Refund button; verified with Playwright against dev)
...
- **M10** test gaps: the Stripe fake didn't model off-session `requires_action`/declines; no coverage for the cancellation fee, ACH cancel-during-processing, or concurrency. Phase A added per-file mocks + tests for its scope. **FIXED-D** (`tests/helpers/stripe.ts` gained `MAGIC_PM` + `fakeOffSessionCharge` — `pm_decline` throws a StripeCardError-shaped error with `payment_intent` attached, `pm_3ds` returns `requires_action`, `pm_bank` returns `processing` — used by the charge-route decline/3DS tests).
...
- **Phase D — observability/UX** (M4 remainder, M7, M10 remainder): SHIPPED — see the FIXED-D statuses above.
