-- 092_cleaner_scorecard_owed_tighten.sql
-- Tighten cleaner_scorecard.owed_now to money we have NOT yet sent the cleaner:
-- status 'pending' or 'approved' only. In the payout lifecycle
-- (pending -> approved -> paid -> bank_paid), 'paid' means the transfer was
-- already sent to their Stripe balance (in transit), so it is not "owed".
-- Everything else in the function is unchanged from migration 091.
