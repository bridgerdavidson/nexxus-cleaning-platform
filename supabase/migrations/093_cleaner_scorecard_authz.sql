-- 093_cleaner_scorecard_authz.sql
-- Harden cleaner_scorecard: enforce the app-level permission model INSIDE the
-- function instead of relying on same-org RLS. The roster + earnings are an
-- app grant (can_manage_cleaners / can_view_payments), not an RLS boundary, so
-- a same-org manager who lacks those could previously call this RPC directly
-- (bypassing the UI gate). Now it is SECURITY DEFINER and authorizes the caller
-- itself: non-members and unprivileged managers get no rows, and earnings/owed
-- are nulled for callers without payment access. (Addresses Codex P1.)
