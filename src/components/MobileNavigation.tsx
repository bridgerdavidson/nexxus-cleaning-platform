  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg border-t border-x border-gray-200 z-40 rounded-t-2xl shadow-[0_-2px_12px_rgba(0,0,0,0.06)] pb-[env(safe-area-inset-bottom)]">
      <div className="flex items-center justify-around px-2 py-2 h-20">
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <span
                  aria-hidden
                  className="absolute bottom-1 h-[3px] w-6 rounded-full bg-primary-600"
                />
              )}
            </button>
          );
        ...
        >
          <Menu className="w-5 h-5 mb-1 transition-colors duration-200 group-hover:text-gray-700" strokeWidth={1.75} />
          <span className="text-[11px] font-medium tracking-wide group-hover:text-gray-700">Menu</span>
        </button>
      </div>
    </nav>
  );
