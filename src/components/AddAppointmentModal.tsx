...
import { X, Calendar, Search, Plus, User, Home, Star, CheckCircle, Loader2 } from "lucide-react";
...
export default function AddAppointmentModal({
  ...
  // Fetch properties when homeowner is selected
  useEffect(() => {
  ...
  const fetchHomeowners = async () => {
    ...
    try {
      ...
      const homeownersData = (members || []).map(member => {
  ...
  const handleCreateAppointment = async () => {
    if (!selectedHomeowner || !selectedProperty || !selectedServiceType || 
        !scheduledDate || !scheduledTime || !currentOrganizationId) {
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden animate-slide-up">
          ...
          <div className="bg-gradient-to-r from-primary-600 to-primary-700 text-white px-8 py-6">
            ...
            <div className="flex justify-center gap-2 mt-4">
              <div className={`h-1 w-16 rounded-full transition-colors ${currentStep >= 1 ? 'bg-white' : 'bg-white/30'}`} />
              <div className={`h-1 w-16 rounded-full transition-colors ${currentStep >= 2 ? 'bg-white' : 'bg-white/30'}`} />
              <div className={`h-1 w-16 rounded-full transition-colors ${currentStep >= 3 ? 'bg-white' : 'bg-white/30'}`} />
            </div>
          </div>
          ...
          <div className="p-8 overflow-y-auto max-h-[calc(90vh-250px)]">
            ...
            {/* Step 1: Select Homeowner & Property */}
            {currentStep === 1 && (
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-gray-600">{homeowner.email}</p>
                              </div>
                            </div>
                            {selectedHomeowner?.id === homeowner.id && (
                    ...
                    {filteredCleaners.map((cleaner) => (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          ...
                          {selectedCleaner?.id === cleaner.id && !skipCleaner && (
                            <CheckCircle className="w-5 h-5 text-primary-600 flex-shrink-0" />
                          )}
                        </div>
                      </button>
