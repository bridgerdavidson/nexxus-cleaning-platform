...
export function OperatorRail({ activeId }: { activeId?: string }) {
  return (
    ...
    >
      {/*
        brand — owns the full-height rail with no divider. ONE lockup image: the
        icon is its left portion and is always visible; the wordmark is clipped
        by the inner wrapper until the rail is hovered, when the wrapper widens
        to "reveal" the wordmark in place. The icon never moves or resizes (no
        crossfade, no swap). Theme via CSS dark: so it's SSR-correct, no flash.
      */}
