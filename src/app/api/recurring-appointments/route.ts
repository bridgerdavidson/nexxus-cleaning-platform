...
interface CreateRecurringAppointmentInput {
  ...
  paymentMethodId?: string | null; // Saved card for the series; charged when each job is completed.
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const appointmentRows = occurrences.map((occ) => ({
      ...
      // Carry the chosen card onto every occurrence; it is charged when each job is completed.
