...
/**
 * Pins the payout-model value-space transition (migration 114 / PR1 of the
 * pay-request feature): the route accepts BOTH spellings of the percentage
 * model but writes the LEGACY one until migration 115 backfills the data.
...
describe('PATCH /api/organizations/[orgId]/profile payout model', () => {
  ...
  it("accepts 'percentage' and writes the legacy spelling (transition until migration 115)", async () => {
